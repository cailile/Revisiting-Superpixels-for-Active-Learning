...
def main(unused_argv):       
    ...
    # load previously selected idx
    selected_idx_prev = []
    for i in range(FLAGS.batch_id):
        selected_idx_prev.append(pickle.load(open(os.path.join(FLAGS.region_idx_dir, 'batch_{}_selected_idx.pkl'.format(i)), 'rb')) )
    if selected_idx_prev != []:
        selected_idx_prev = np.hstack(selected_idx_prev)  
    ...
    if FLAGS.is_bal == 'True':
        ...
        region_num_per_class = np.zeros((num_class,), dtype=np.int)
        total_region_num = 0.0
        for i in range(num_class):
            region_num_per_class[i] = len(class_to_region_idx[i])
            total_region_num += region_num_per_class[i]
        
        p = np.zeros((num_class,), dtype=np.float32)
        w = np.zeros((num_class,), dtype=np.float32)
        for i in range(num_class):
            p[i] = region_num_per_class[i]/total_region_num   
            w[i]= np.exp(-p[i]) 
            
        region_to_w = np.ones((region_num,1), dtype=np.float32)
        for i in range(num_class):
            for reg in class_to_region_idx[i]:
                region_to_w[reg] = w[i]
